...
function toScript(x: LockOrType): Script {
  return new Script(
    x.codeHash,
    x.args,
    x.hashType === 'type' ? HashType.type : HashType.data,
  )
}
...
function toCell(x: InputCellsEntity | OutputsEntity): Cell {
  return new Cell(
    ...
    'outPoint' in x && x.outPoint
      ? new OutPoint(x.outPoint.txHash, x.outPoint.index)
      : undefined,
    x.data,
  )
}
